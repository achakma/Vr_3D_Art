...
if (isset($_FILES['images'])) {
    ...
    for ($i = 0; $i < $total_count; $i++) {
        ...
        if (empty($errors)) {
            ...
            $file_path = "C:/xampp/htdocs/Virtual-Reality-Museum-master/textures/ex_img/" . $unique_file_name;
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $file_path = "C:/xampp/htdocs/Virtual-Reality-Museum-master/textures/ex_img/" . $file_name;
            if (file_exists($file_path)) {
                // Move the file to the folder
                $new_path = "C:/xampp/htdocs/Virtual-Reality-Museum-master/textures/ex_img/" . $file_name;
...
if (mysqli_affected_rows($conn) > 0) {
    ...
    echo "No records deleted from database.";
}
}
